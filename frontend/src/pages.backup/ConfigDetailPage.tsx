...
import { useConfig, useApproveConfig, useDeployConfig } from '@/hooks/useConfigs';
...
export function ConfigDetailPage() {
  ...
  const canApprove = config.status === 'in_review' && (user?.role === 'lead_designer' || user?.role === 'product_manager' || user?.role === 'admin');
  const canDeploy = config.status === 'approved' && (user?.role === 'product_manager' || user?.role === 'admin');
    ...
    <div className="p-8 space-y-8">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Workflow actions for this configuration</CardDescription>
        </CardHeader>
        <CardContent className="flex gap-4">
          {canEdit && (
            <Button onClick={() => navigate(`/configs/${configId}/edit`)}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <Card>
          ...
          <CardContent className="space-y-2">
            ...
            <div className="flex justify-between">
              <span className="text-muted-foreground">Notification Config:</span>
              <span className="font-medium">
